...
import styles from './preview-a.module.css';

export const metadata: Metadata = { title: 'Dashboard preview A — GTS' };
export const dynamic = 'force-dynamic';

const LEAD_ALERT_COUNT = 3;

/** Same tone the alert already carries server-side — no re-derivation. */
...
export default async function DashboardPreviewA() {
        ...
        <section className={styles.section}>
          ...
          {data.alerts.length === 0 ? (
            <p className={styles.allClear}>
              <strong>{dict.overview.dashboard.nothingNeedsYouTitle}</strong>{' '}
            ...
            <>
              <ul className={styles.alertList}>
                {leadAlerts.map((alert) => (
                  <li key={alert.id} className={styles.alertRow} data-tone={alert.tone}>
            ...
            {data.projects.length === 0 ? (
              ...
              <ul className={styles.workList}>
                {data.projects.slice(0, 5).map((p) => {
                  ...
                  return (
                    <li key={p.id}>
                      <a href={`/projects/${p.id}`} className={styles.workRow}>
                  ...
                  <>
                    <div className={styles.attendanceStrip}>
                      <div>
                        ...
                        <p className={styles.attendanceFigure}>{data.attendanceToday.present}</p>
                      </div>
                      <div>
